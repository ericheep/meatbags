...
private:
	...
	ofVboMesh gridMesh;
	float     lastGridScale;
	ofPoint   lastGridOrigin;

	// connection string cache
	void           rebuildConnectionStrings(const vector<Sensor*>& sensors);
	vector<string> cachedConnectionStrings;
	int            lastSensorCount;

	// point cloud mesh - track last count to avoid index rebuild
	int lastNumberLidarPoints;
};
